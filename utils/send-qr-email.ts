...
export async function sendQREmail(entry: Entry): Promise<void> {
  const qrDataUrl = await QRCode.toDataURL(entry.id, {
    width: 280,
    margin: 2,
  ...
  <div style="max-width:480px;margin:40px auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb;">
    ...
    <div style="padding:32px;text-align:center;">
      ...
      <p style="font-size:14px;color:#6b7280;margin:0 0 28px;">
        Your VIP pass is ready. Present the QR code below at the entrance.
      </p>

      <div style="display:inline-block;padding:12px;border-radius:16px;border:1px solid #e5e7eb;background:#fff;">
        <img src="${qrDataUrl}" width="220" height="220" alt="Your QR Code" style="display:block;border-radius:8px;" />
      </div>

      <table style="margin:28px auto 0;font-size:14px;color:#374151;border-collapse:collapse;">
        <tr>
          <td style="padding:4px 12px 4px 0;color:#9ca3af;">Name</td>
          <td style="padding:4px 0;font-weight:600;">${entry.name}</td>
        </tr>
        <tr>
          <td style="padding:4px 12px 4px 0;color:#9ca3af;">Email</td>
          <td style="padding:4px 0;">${entry.email}</td>
        </tr>
        <tr>
          <td style="padding:4px 12px 4px 0;color:#9ca3af;">Phone</td>
          <td style="padding:4px 0;">${entry.number}</td>
        </tr>
      </table>
    </div>
    ...
    <div style="background:#f9fafb;padding:20px 32px;text-align:center;border-top:1px solid #e5e7eb;">
      <p style="font-size:13px;color:#6b7280;margin:0 0 6px;">
        ...
        <a href="tel:9542760910" style="color:#7c3aed;text-decoration:none;font-weight:600;">9542760910</a>
      </p>
